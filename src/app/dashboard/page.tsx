    ...
    <div style={{ padding: 'var(--s-10) var(--s-8)' }}>
      <div style={{ marginBottom: 'var(--s-10)' }}>
        ...
        <p style={{ color: 'var(--fg-muted)', fontSize: 'var(--t-body-sm)' }}>
          Hola, {session.user.name?.split(' ')[0]}. Acá están tus listings.
        </p>
      </div>
            ...
            >
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--s-3)', marginBottom: 'var(--s-2)' }}>
                  <h2 style={{ fontSize: 'var(--t-h4)', fontWeight: 500, margin: 0 }}>
                    {listing.name}
                  </h2>
                ...
                <div style={{ display: 'flex', gap: 'var(--s-6)' }}>
                  ...
                  {reviewStats.count > 0 && (
                    <StatItem
                      label="Rating"
                      value={`${reviewStats.averageRating.toFixed(1)}/10`}
                    />
                  )}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--s-2)', alignItems: 'flex-end' }}>
                <Link
                  href={`/dashboard/listing/${listing.id}/editar`}
...
function EmptyState() {
  return (
    ...
    >
      ...
      >
        Todavía no tenés listings
      </p>
      <p style={{ color: 'var(--fg-muted)', fontSize: 'var(--t-body-sm)' }}>
        Creá tu primera ficha y aparecé en el directorio.
      </p>
...
function StatusBadge({ status }: { status: string }) {
  ...
  const c = config[status] ?? config[ListingStatus.DRAFT]
  return (
    <span
      style={{
